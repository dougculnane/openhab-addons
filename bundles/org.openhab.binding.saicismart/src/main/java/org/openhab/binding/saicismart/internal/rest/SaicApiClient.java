...
public class SaicApiClient {
    ...
    public VehicleList getVehicleList(String token)
            throws IOException, InterruptedException, TimeoutException, ExecutionException {
        return sendRequest(new VehicleList(), URL_ROOT_V1 + "vehicle/list", HttpMethod.GET, "", "application/json",
                token, "");
    }

    public VehicleStatus getVehicleStatus(String token, String vin)
            throws IOException, InterruptedException, TimeoutException, ExecutionException {
        return sendRequest(new VehicleStatus(),
                URL_ROOT_V1 + "vehicle/status?vin=" + HashUtils.sha256(vin) + "&vehStatusReqType=2", HttpMethod.GET, "",
                "application/json", token, "");
    }

    public VechicleChargingMgmtData getVehicleChargingMgmtData(String token, String vin)
            throws IOException, InterruptedException, TimeoutException, ExecutionException {
        VechicleChargingMgmtData message1 = sendRequest(new VechicleChargingMgmtData(),
                URL_ROOT_V1 + "vehicle/charging/mgmtData?vin=" + HashUtils.sha256(vin), HttpMethod.GET, "",
                "application/json", token, "");
        if (message1.isSuccess()) {
            return sendRequest(new VechicleChargingMgmtData(),
                    URL_ROOT_V1 + "vehicle/charging/mgmtData?vin=" + HashUtils.sha256(vin), HttpMethod.GET, "",
    ...
    public VehicleCcInfo getVehicleCcInfo(@Nullable String token, String vin)
            throws IOException, InterruptedException, TimeoutException, ExecutionException {
        VehicleCcInfo message1 = sendRequest(new VehicleCcInfo(),
                URL_ROOT_V1 + "vehicle/vehicle/ccInfo?vin=" + HashUtils.sha256(vin), HttpMethod.GET, "",
                "application/json", token, "");
        if (message1.isSuccess()) {
            return sendRequest(new VehicleCcInfo(), URL_ROOT_V1 + "vehicle/vehicle/ccInfo?vin=" + HashUtils.sha256(vin),
                    HttpMethod.GET, "", "application/json", token, message1.getEventId());
        }
        return message1;
    }

    public VehicleLocation getVehicleLocation(@Nullable String token, String vin)
            throws IOException, InterruptedException, TimeoutException, ExecutionException {
        VehicleLocation message1 = sendRequest(new VehicleLocation(),
                URL_ROOT_V1 + "vehicle/location?vin=" + HashUtils.sha256(vin), HttpMethod.GET, "", "application/json",
                token, "");
        if (message1.isSuccess()) {
            return sendRequest(new VehicleLocation(), URL_ROOT_V1 + "vehicle/location?vin=" + HashUtils.sha256(vin),
                    HttpMethod.GET, "", "application/json", token, message1.getEventId());
        }
        return message1;
    }
    ...
    private <T extends JsonResponseMessage> T sendRequest(T responseMessage, String url, HttpMethod httpMethod,
            String requestBody, String contentType, String token, String eventId)
            throws IOException, InterruptedException, TimeoutException, ExecutionException {
        ...
        Request request = this.httpClient.newRequest(endpoint).method(httpMethod)
                .header("tenant-id", APIConfig.TENANT_ID).header("user-type", APIConfig.USER_TYPE)
                .header("app-send-date", "" + appSendDate).header("app-content-encrypted", APIConfig.CONTENT_ENCRYPTED)
                .header("Authorization", APIConfig.PARAM_AUTHENTICATION).header("original-content-type", contentType);

        String encryptedBody = "";
        if (requestBody != null && !requestBody.isBlank()) {
            encryptedBody = EncryptionUtils.encryptRequest(endpoint.toString(), appSendDate, APIConfig.TENANT_ID, token,
        ...
        request.header("app-verification-string", EncryptionUtils.calculateRequestVerification(replace, appSendDate,
                APIConfig.TENANT_ID, contentType, encryptedBody, token));
